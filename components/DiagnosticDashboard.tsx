...
const Placeholder: React.FC = () => (
  <div className="h-full flex flex-col items-center justify-center text-center p-6 bg-[var(--surface-1)] rounded-lg border border-[var(--surface-border)] shadow-xl shadow-black/40">
    <GaugeIcon className="w-16 h-16 text-gray-600 mb-4" />
    <h3 className="text-lg font-bold font-display text-gray-300">Digital Dashboard</h3>
...
const Section: React.FC<{ icon: React.ReactNode; title: string; children: React.ReactNode; className?: string }> = ({ icon, title, children, className = '' }) => (
  <div className={`bg-black/20 p-4 rounded-lg border border-gray-700/50 ${className}`}>
    <div className="flex items-center mb-3">
      {icon}
      <h4 className="text-sm font-semibold font-display text-gray-300 uppercase tracking-wider">{title}</h4>
    </div>
    {children}
  </div>
);
