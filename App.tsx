...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const diagnosisMsg = messages.find(
      (msg) => msg.role === 'model' && msg.content.startsWith('### ✅ Final Diagnosis:')
    );
    if (diagnosisMsg) {
      setFinalDiagnosis(diagnosisMsg);
    }
  }, [messages]);
  ...
  const loadSession = (sessionId: string) => {
    ...
    if (sessionToLoad) {
      ...
      const diagnosisMsg = sessionToLoad.messages.find(
          (msg) => msg.role === 'model' && msg.content.startsWith('### ✅ Final Diagnosis:')
      );
      setFinalDiagnosis(diagnosisMsg || null);
    }
    setIsSidebarOpen(false);
    setCurrentView('chat');
  };
  ...
  const renderCurrentView = () => {
    switch(currentView) {
      case 'guide':
        return <WorkshopGuide onClose={() => setCurrentView('chat')} />;
      case 'privacy':
        return <PrivacyPolicy onClose={() => setCurrentView('chat')} />;
      case 'chat':
      default:
        return (
          <div className="flex flex-col flex-1 h-full">
            <header className="flex items-center justify-between p-3 sm:p-4 border-b border-[var(--surface-border)] shadow-md shrink-0">
              ...
              <div className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-4 p-4 overflow-hidden">
                {/* Main Chat Column */}
                <div className="lg:col-span-3 flex flex-col h-full min-h-0 bg-[var(--surface-1)]/80 backdrop-blur-md rounded-lg border border-[var(--surface-border)] shadow-2xl shadow-black/40">
                  ...
                  <footer className="p-3 sm:p-4 border-t border-[var(--surface-border)] shrink-0">
                    <div className="max-w-3xl mx-auto">
                      ...
                      <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className={`flex items-center space-x-2 sm:space-x-3 bg-slate-800/80 rounded-xl border border-[var(--surface-border)] focus-within:ring-2 focus-within:ring-[var(--accent-primary)] transition-shadow p-1 ${isChatDisabled ? 'opacity-60' : ''}`}>
                        ...
                        >
                          {isChatDisabled && !!apiKey ? <div className="w-6 h-6 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : <SendIcon className="w-6 h-6" />}
                        </button>
                      </form>
                    </div>
                  </footer>
                </div>
                {/* Diagnostic Dashboard Column */}
                <div className="hidden lg:block lg:col-span-1 h-full min-h-0">
                    <DiagnosticDashboard diagnosis={finalDiagnosis} />
                </div>
              </div>
          </div>
        );
